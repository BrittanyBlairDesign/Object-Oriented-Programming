...
void World::GenerateWorld()
{
    ...
    m_ppGrid[lastIndex] = Tile::CreateTile(TileClass::kBomb);  // special tile for ending the level; there is only one of these
    m_ppGrid[0] = Tile::CreateTile(TileClass::KFloor);  // guarantee that the starting location is open
    ...
    m_ppGrid[randomTile] = Tile::CreateTile(TileClass::KMimic);
	...
	for (int tileIndex = 1; tileIndex < (m_width * m_height) - 1; ++tileIndex)
	{
		...
		switch (s_tileProbabilities[probabilityIndex].second)
		{
            case TileType::k_floor:
                m_ppGrid[tileIndex] = Tile::CreateTile(TileClass::KFloor);
				break;

            case TileType::k_bomb:
                m_ppGrid[tileIndex] = Tile::CreateTile(TileClass::kBomb);
				break;

            case TileType::k_treasure:
                m_ppGrid[tileIndex] = Tile::CreateTile(TileClass::KTreasure);
				break;

            case TileType::k_mimic:
                m_ppGrid[tileIndex] = Tile::CreateTile(TileClass::KMimic);
                break;
